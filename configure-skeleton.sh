...
echo
files=$(grep -E -r -l -i ":author|:vendor|:package|:namespace|spatie|tool" --exclude-dir=vendor ./*  | grep -v "$script_name")
...
for file in $files ; do
    ...
    | sed "s/:namespace_vendor/$vendor_name/g" \
    | sed "s/_tool_/$package_name_underscore/g" \
    | sed "s/tool/$package_name/g" \
    | sed "s/:namespace_tool_name/$class_name/g" \
    | sed "s/:package_description/$package_description/g" \
    | sed "/^\*\*Note:\*\* Run/d" \
    > "$temp_file"
    rm -f "$file"
    new_file=`echo $file | sed -e "s/Tool/${class_name}/g"`
    mv "$temp_file" "$new_file"
done
mv "./config/tool.php" "./config/${package_name}.php"
mv "./database/migrations/create_tool_table.php.stub" "./database/migrations/create${package_name_underscore}table.php.stub"
